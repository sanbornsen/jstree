...
class tree
{
	...
	public function new_products($id, $num){
		$sql = "SELECT nm FROM tree_data WHERE id = ".$id;
		$name = $this->db->one($sql);
		$name = str_replace("(", "", trim($name));
		$name = explode("-",trim(end(explode(":",$name))));
		$lower = $name[0];
		$upper = $name[1];
		$new_child = [];
		if($upper-$lower < (int)$num)
			$num = $upper-$lower;
		for($i=0;$i<(int)$num;$i++){
			$nc = rand($lower,$upper);
			while (in_array($nc, $new_child)) {
				$nc = rand($lower,$upper);
			}
			$new_child[] = $nc;
		}

		$sql = "SELECT id FROM tree_struct WHERE tree_struct.pid = ".$id;
		$ids = $this->db->all($sql);
		foreach ($ids as $id) {
			$sql = "DELETE FROM tree_data WHERE id = ".$id;
			$this->db->query($sql);
		}
		$sql = "DELETE FROM tree_struct WHERE pid = ".$id;
		$this->db->query($sql);
		return json_encode($new_child);
	}
